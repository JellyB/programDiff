...
// },
  ...
  onQuery: function() {
    ...
    .then(res=>{
      ...
      items.forEach(element => {
        console.log(element);
        element.tel = element.tel.replace(pat,'$1****$2');
      });
      this.setData({
        historys: items
      },()=>{
  ...
  onQuery2: function() {
    ...
    .catch(err=>{
      console.error('[云函数] [fetchSome] 调用失败', err)
      wx.navigateTo({
        url: '../deployFunctions/deployFunctions',
      })
    })
  ...
  getSomeNum: function() {
    ...
    .then(res=>{
      console.log('[云函数] [getSomeNum]: ', res)
      let total = res.result.total;
      // let arr = items.concat(items,items,items,items,items,items,items);
      this.setData({
        sortNo: total+1
    ...
    .catch(err=>{
      console.error('[云函数] [getSomeNum] 调用失败', err)
      wx.navigateTo({
        url: '../deployFunctions/deployFunctions',
      })
    })
